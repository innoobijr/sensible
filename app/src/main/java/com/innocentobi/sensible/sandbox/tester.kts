 ...
 *
 * A higher king is essential a generic type that can be a type constructor. The syntax looks like this:
 * Kind[F, A] := where F is the functor and A is the type with its type constructor
 * Another way to think of it is think of the F, as containing a type A: But we dont want to specify
 * the type: So for example:
 * 1. Option[A]
 * 2. Either[A]
 * 3. List[A]
 *
 * In this example, Option, Either, and Lists are functors with a type constructor
 **/